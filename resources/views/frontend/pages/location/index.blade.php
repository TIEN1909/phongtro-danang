...
@section('title', 'Quận huyện ' . $location->name)
@push('css')
...
<div class="info">
    <h1>{{ $location->ten }}</h1>
    <div class="description">{{ $location->mota }}</div>
</div>
            ...
            <div class="card">
                ...
                <section class="section section-top-location">
                    <ul class="location-district phongtro clearfix">
                        ...
                        <li>
                            <a class="district-item" title="Phòng trọ {{ $item->ten }}"
                                href="{{ route('get.room.by_district',['id' => $item->id, 'slug' => $item->slug]) }}">{{ $item->ten }}</a>
